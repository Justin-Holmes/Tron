...
function Game() {
  this.playerOne    = new Player("red", 10, 30, "right");
  this.playerTwo    = new Player("blue", 110, 30, "left");
  this.playerTrails = [];
}
...
Game.prototype.start = function () {
  ...
  $(document).keydown(function(k){
    ...
    playerKeys(key, oneKeys, this.playerOne);
    playerKeys(key, twoKeys, this.playerTwo);
  });
};
...
Game.prototype.detectCollision = function(xPosition, yPosition) {
  ...
  for (var i = 0; i < this.playerTrails.length; i++) {
    if (xPosition == this.playerTrails[i][0] && yPosition == this.playerTrails[i][1]) {
      clearInterval(window.gameLoopInterval);
      this.end();
    }
  }
